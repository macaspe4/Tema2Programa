...
**Ejercicio**: Accede al Instituto Nacional de Estadística y busca en
INEbase la población de todos los municipios de la provincia de
Valencia. Descarga los ficheros de datos en todos los formatos que estén
disponibles y guárdalos en una carpeta *'./data'*.

Ruta: Inebase -\>Demografía-\>Padrón -\> Población por municipios
-\>Detalle municipal
...
El fichero Json no se puede descargar directamente con \`download.file
ya que realiza una consulta. Observar que el formato es distinto. Se
puede descargar directamente desde la página web pulsando sobre el
enlace. (**2020** Sí que se descarga directamente con el enlace)

Visualizar el contenido del fichero con extensión **.px** ¿Se ven
correctamente los acentos ?. Depende del SO que estés usando ya que la
codificación es ISO-8859-1. Al abrir con Rstudio, por defecto se abre
con la codificación que se haya especificado en la configuración
(**tools-\>Global Options-\>Code -\> Saving**), pero se puede reabrir
usando una codificación distinta (**Reopen with encoding**), pero *¿
Cómo podemos saber cuál es la codificación correcta ?*
...
Lectura correcta que considera las comas dentro de las comillas como
separador de campos. Podemos especificar qué comillas se han usado para
separar caracteres
...
Lectura incorrecta que considera las comas dentro de las comillas como
separador de campos, ya que hemos indicado un caracter de separación
erróneo. Como consecuencia no se reconocen las cadenas y las comas son
separadores, pero **ATENCIÓN, NO PRODUCE NINGÚN ERROR DE EJECCUCION** ¿
Cómo se está realizando la importación ?
...
## Lectura fichero *Unclaimed_bank_accounts.csv*

Usando **readLines** cargamos un fichero de texto en un vector de
caracteres donde cada elemento es una línea.
...
## Ejercicio. Importación de los ficheros *UCIautoMpg.txt* y *auto-mpg.names*

El objetivo de este ejercicio es importar los datos del fichero
*UCIautoMpg.txt* y poner nombres a las variables cuya información está
en *auto-mpg.names*.

Proceso:

1.  División en 2 vectores de caracteres usando el caracter
    tabulador'\t'
...
Usamos el conjunto `mtcars` y hacemos una gráfica cualquiera. Podemos
usar una carpeta para almacenar las figuras.
...
Si queremos almacenar muchos gráficos en fichero no damos nombres y se
van nombrando secuencialmente (**RplotXXX**). Para poder especificar la
ruta de almacenamiento se ha utilizado **setwd()**
...
En este ejemplo se ilustra cómo acceder (y en este caso también crear)
una base de datos desde R.

Al hacer la conexión como no existe la BBDD se creará (lo habitual será
que nos faciliten los parámetros de acceso a la BBDD). Este sería un
acceso a una BBDD remota real, donde el *password* se ha omitido.

library(RMySQL) library(DBI) drv = dbDriver("MySQL") mydb_conn =
dbConnect(drv, user='aruni_db', password='XXXXXXXX', dbname='aranco',
host='127.0.0.1', port=8080, client.flag = CLIENT_MULTI_STATEMENTS)
